...
using System.IO;//包含操作文件系统所需的所有类和方法
using System;
using System.Xml;
...
public class DataManager : MonoBehaviour,IManager
{
    ...
    public void ReadFromFile(string filename)//读文件数据
    {
    ...
    public void DeleteFile(string filename)//删除文件文本,未使用
    {
        if(!File.Exists(filename))
        {
    ...
    private void WriteToStream(string filename)//写数据流
    {
        if (!File.Exists(filename))
        {
            ...
            //newStream.Close();行开头使用using 关键字可自动关闭数据流
    ...
    public void WriteToXML(string filename)
    {
        if (!File.Exists(filename))
        {
            ...
            for (int i = 1; i < 5; i++)
            {
                xmlWriter.WriteElementString("level", "Level-" + i);//标签名,内容
